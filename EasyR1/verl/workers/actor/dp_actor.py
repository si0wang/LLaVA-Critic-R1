...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch: Dict[str, torch.Tensor], temperature: float) -> torch.Tensor:
        ...
        # ---- Critical: make attention mask boolean for SDPA ----
        if attention_mask.dtype != torch.bool:
            attention_mask = attention_mask.ne(0)

        # ---- Multi-modal inputs dtype normalization ----
        ...
        if "multi_modal_inputs" in micro_batch:
            ...
            for i in range(len(mm_list)):
                for key in ("pixel_values", "aspect_ratio_ids", "aspect_ratio_mask", "num_tiles"):
                    ...
                    # Key-specific dtype rules (avoid blanket bf16 casts)
                    if key == "pixel_values":
                        # Keep preprocessing in float32; AMP will cast for compute
                        if (not x.dtype.is_floating_point) or (x.dtype != torch.float32):
                            x = x.to(torch.float32)
                    elif key == "aspect_ratio_mask":
                        # SDPA prefers bool masks for universal compatibility
                        if x.dtype != torch.bool:
                            x = x.ne(0)
                    else:
                        # ids/counters for embedding/indexing
                        if x.dtype != torch.long:
                            x = x.to(torch.long)

                    mm_list[i][key] = x

            # Concatenate per-key across samples in micro-batch
            # Assume each dict in mm_list shares the same keys subset
