...
def eval_model(args):
    ...
    for chunk in tqdm(split_list(questions, math.ceil(len(questions) / batch_size))):
        ...
        for data in chunk:
            ...
            # 只有在 is_thinking 时才追加显式 <think>，否则不加
        ...
        for out, data in zip(outputs, valid_datas):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--is-thinking", action="store_true", help="Append the thinking prompt and add an explicit <think> tag")
    parser.add_argument("--is-mimo-thinking", action="store_true", help="Append the thinking prompt and add an explicit <think> tag")
