...
def eval_model(args):
    ...
    questions = list(load_dataset("lmms-lab/ChartQA", split="test"))
    ...
    for chunk in tqdm(split_list(questions, math.ceil(len(questions) / batch_size))):
        ...
        for data in chunk:
            ...
            qs = data['question'] + "\nAnswer the question using a single word or number."
            if args.is_thinking:
                qs += instruct_prompt
            ...
            message = [
                {"role": "system", "content": "You are a helpful assistant."},
                {
                    "role": "user",
                    "content": [
                        {"type": "image", "min_pixels": 224*224, "max_pixels": 1280*28*28},
                        {"type": "text", "text": qs},
                    ],
                },
            ]
        ...
        for out, data in zip(outputs, valid_data):
