...
def eval_model(args):
    ...
    df = pd.read_excel('/fsx-project/xywang96/ThinkLite-VL/MMVP/Questions.xlsx', engine='openpyxl')
    ...
    for chunk in tqdm(split_list(questions, math.ceil(len(questions) / batch_size))):
        batch_inputs = []
        for data in chunk:
            img = Image.open(f"/fsx-project/xywang96/ThinkLite-VL/MMVP/MMVP Images/{data['Index']}.jpg")
            if img.height < 28 or img.width < 28:
                print('Skip sample')
                continue

            qs = data['Question'] + data['Options']
            qs += " Please answer directly with only the letter of the correct option."
            if args.is_thinking:
                qs += instruct_prompt
