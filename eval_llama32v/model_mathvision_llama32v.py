...
cap_instruct_prompt = r"You FIRST describe the given image in a detailed manner, THEN think about the reasoning process as an internal monologue based on the generated description and given question and then provide the final answer. The description MUST BE enclosed within <caption> </caption> tags. The reasoning process MUST BE enclosed within <think> </think> tags. The final answer MUST BE put in \boxed{}."
...
def eval_model(args):
    ...
    for chunk in tqdm(split_list(questions, math.ceil(len(questions) / batch_size))):
        ...
        for data in chunk:
            ...
            # use thinking prompt only when requested
            if args.is_thinking:
        ...
        for out, data in zip(outputs, valid_chunk):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--is-thinking", action="store_true", help="Whether to append the thinking prompt")
    parser.add_argument("--is-mimo-thinking", action="store_true", help="Whether to append the thinking prompt")
