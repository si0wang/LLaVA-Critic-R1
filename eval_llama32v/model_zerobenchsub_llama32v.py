...
def eval_model(args):
    ...
    questions = list(load_dataset("jonathan-roberts1/zerobench", split="zerobench_subquestions"))
    ...
    for chunk in tqdm(split_list(questions, math.ceil(len(questions) / batch_size))):
        ...
        for data in chunk:
            imgs = []
            for img in data['question_images_decoded']:
                imgs.append(img)

            qs = data['question_text']
            if args.is_thinking:
                qs += instruct_prompt
            ...
            message = [
                ...
                {"role": "user", "content": [*placeholders, {"type": "text", "text": qs}]},
            ]
