...
def eval_model(args):
    ...
    questions = []
    with open("/fsx-project/xywang96/ThinkLite-VL/VisuLogic/data.jsonl", 'r') as files:
        for line in files:
    ...
    for chunk in tqdm(split_list(questions, math.ceil(len(questions) / batch_size))):
        ...
        valid_inputs = []
        for data in chunk:
            img = Image.open(f"/fsx-project/xywang96/ThinkLite-VL/VisuLogic/{data['image_path']}")
